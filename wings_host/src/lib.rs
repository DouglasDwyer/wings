...
impl<H: Host> Events<H> {
    ...
    pub const fn with<T: HostEvent>(&'static self) -> Self {
        Self {
            ...
            event_raisers: self.event_raisers.push(HostEventRaiser {
                ty: T::TYPE,
                raise: Self::raise::<T>,
            }),
        }
...
impl<H: Host, S: GeeseSystem> Traits<H, S> {
    ...
    pub const fn with<T: Proxyable + SystemTrait + ?Sized>(&'static self) -> Self
    where
        S: AsMut<T>,
    {
        Self {
            inner: self.inner.push(HostSystemTrait {
                ty: T::TYPE,
                invoke: Self::invoke::<T>,
            }),
            marker: PhantomData,
        }
    }
    ...
    fn invoke<T: Proxyable + SystemTrait + ?Sized>(
        ctx: &mut GeeseContextHandle<WingsHost<H>>,
        func_index: u32,
        buffer: &mut Vec<u8>,
    ) -> Result<(), WingsError>
    where
        S: AsMut<T>,
    {
        unsafe { ctx.get_mut::<S>().as_mut().invoke(func_index, buffer) }
    }
}
...
impl WingsImage {
    ...
    pub fn add<T: wings_marshal::exported_type::ExportType + ?Sized>(
        &mut self,
        module: &WingsModule,
    ) {
    ...
    fn add_top_level_systems<T: wings_marshal::exported_type::ExportType + ?Sized>(
        &mut self,
        module: &WingsModule,
    ) {
        let group_ty = ExportedType::from(T::TYPE);
        if let Some(group) = module
            .0
            .metadata
            .group_instantiates
            .iter()
            .find(|x| x.group_ty == group_ty)
        {
...
impl<H: Host> WingsHost<H> {
    ...
    fn check_group_instantiates_duplicates(
        &self,
        group_instantiates: &[InstantiateGroup],
    ) -> Result<(), WingsError> {
        for group in group_instantiates {
            for i in 1..group.systems.len() {
                for j in 0..i {
                    if group.systems[i] == group.systems[j] {
                        return Err(WingsError::from_invalid_module(format!(
                            "Duplicate instantiated systems in group {}",
                            group.group_ty.name
                        )));
                    }
                }
    ...
    fn collate_group_instantiates(&self, group_instantiates: &mut Vec<InstantiateGroup>) {
        ...
        while i < group_instantiates.len() {
            ...
            if let Some(index) = group_instantiates[..i]
                .iter()
                .position(|x| x.group_ty == current.group_ty)
            {
    ...
    fn drop_systems(&mut self) -> Result<(), WingsError> {
        ...
        for i in (0..store.data().systems.len()).rev() {
            ...
            let invoke_func = store.data().instance_funcs[system.instance as usize]
                .invoke_func_1
                .clone();
            invoke_func
                .call(
                    &mut store,
                    &[
                        Value::I32(system.drop_fn.into()),
                        Value::I32(system.pointer.into()),
                    ],
                    &mut [Value::I32(0)],
                )
                .map_err(WingsError::from_trap)?;
        }
    ...
    fn load_module_metadata(&self, module: &Module) -> Result<ModuleMetadata, WingsError> {
        ...
        let instance = Instance::new(&mut store, module, &dummy_imports)
            .map_err(WingsError::from_invalid_module)?;
        ...
        let Some(Extern::Memory(memory)) = instance.get_export(&mut store, "memory") else {
            return Err(WingsError::from_invalid_module("Module missing memory"));
        };
        ...
        for func in metadata.instantiates {
            ...
            group_instantiates
                .push(bincode::deserialize(&buffer).map_err(WingsError::from_invalid_module)?);
        }
    ...
    fn create_dummy_imports<C: AsContextMut>(
        &self,
        mut ctx: C,
        module: &Module,
    ) -> Result<Imports, WingsError> {
        let mut imports = Imports::new();
        for import in module.imports(&self.engine) {
            let func = match import.ty {
                ExternType::Func(func_ty) => Func::new(&mut ctx, func_ty, |_, _, _| {
                    Err(WingsError::from_invalid_module("Module called stub method").into())
                }),
                _ => {
                    return Err(WingsError::from_invalid_module(
                        "Module imported unexpected object",
                    ))
                }
            };
    ...
    fn fill_event_handlers(&mut self, image: &WingsImage, types: &[DisjointExportedType]) {
        ...
        for (id, ty) in types.iter().enumerate() {
            ...
            let descriptor = &image.modules[entry.module_id as usize]
                .0
                .metadata
                .system_descriptors[entry.system_id as usize];
            for event_handler in &descriptor.event_handlers {
                let event_entry = data
                    .guest_event_handlers
                    .entry(event_handler.ty.clone())
                    .or_default();
                if !event_entry.instances.contains(&entry.module_id) {
                    event_entry.instances.push(entry.module_id);
                }
                event_entry.event_handlers.push(EventHandlerDescriptor {
                    system: id as u32,
                    event_func: event_handler.event_func,
                    invoke_func: event_handler.invoke_func,
                });
            }
        }
    }
    ...
    fn fill_system_traits(
        &mut self,
        image: &WingsImage,
        types: &[DisjointExportedType],
    ) -> Result<FxHashMap<DisjointExportedType, u32>, WingsError> {
        let store = self.store.as_mut().expect("Failed to get store");
        let data = store.data_mut();
        ...
        for invoker in &data.invokers {
            ...
            data.system_traits
                .push(SystemTraitHolder::Host { invoker: id });
        ...
        for ty in types {
            ...
            for system_trait in &descriptor.traits {
                ...
                let id = data.system_traits.len() as u32;
                data.system_traits.push(SystemTraitHolder::Guest {
                    invoke: system_trait.invoke,
                    system: entry.module_id,
                    v_table: system_trait.v_table,
                });
                data.system_trait_map.insert(ty.clone(), id);
                if trait_map.insert(ty, id).is_some() {
                    return Err(WingsError::from_invalid_module(
    ...
    fn instantiate_modules(&mut self, image: &WingsImage) -> Result<(), WingsError> {
        ...
        for (index, module) in image.modules.iter().enumerate() {
            ...
            let instance = Instance::new(&mut store, &module.0.module, &imports)
                .map_err(WingsError::from_invalid_module)?;
            let Some(Extern::Memory(memory)) = instance.get_export(&mut store, "memory") else {
                unreachable!()
            };
    ...
    fn instantiate_systems(
        &mut self,
        image: &WingsImage,
        order: &[DisjointExportedType],
        trait_map: &FxHashMap<DisjointExportedType, u32>,
    ) -> Result<(), WingsError> {
        let mut store = self.store.as_mut().expect("Failed to get store");
        for ty in order {
            let entry = image.systems[ty];
            ...
            for dependency in &descriptor.dependencies {
                let Some(index) = trait_map.get(&dependency.into()) else {
                    return Err(WingsError::from_invalid_module("Missing dependency"));
                };
                dependencies.push(match store.data().system_traits[*index as usize] {
                    SystemTraitHolder::Guest {
                        system, v_table, ..
                    } => {
                        let system = &store.data().systems[system as usize];
                        if system.instance == entry.module_id {
                            DependencyReference::Local(FatGuestPointer::new(
                                system.pointer,
                                v_table,
                            ))
                        } else {
            ...
            let dependency_data =
                bincode::serialize(&dependencies).map_err(WingsError::Serialization)?;
            let mut result = [Value::I32(0)];
            instance_funcs
                .alloc_marshal_buffer
                .call(
                    &mut store,
                    &[Value::I32(dependency_data.len() as i32)],
                    &mut result,
                )
                .map_err(WingsError::from_invalid_module)?;
            let [Value::I32(pointer)] = result else {
                unreachable!()
            };
            memory
                .write(&mut store, pointer as usize, &dependency_data)
                .map_err(WingsError::from_invalid_module)?;
            instance_funcs
                .invoke_func_1
                .call(
                    &mut store,
                    &[Value::I32(descriptor.new_func.into()), Value::I32(pointer)],
                    &mut result,
                )
                .map_err(WingsError::from_trap)?;
            let [Value::I32(pointer)] = result else {
    ...
    fn metadata_functions<C: AsContextMut>(
        &self,
        mut ctx: C,
        instance: &Instance,
    ) -> MetadataFunctions {
        ...
        for export in instance.exports(&mut ctx) {
            let Extern::Func(func) = export.value else {
                continue;
            };
    ...
    fn read_marshal_buffer<C: AsContextMut>(
        &self,
        mut ctx: C,
        func: &Func,
        memory: &Memory,
        ...
        func.call(&mut ctx, &[], &mut result_pointer_value)
            .map_err(WingsError::from_invalid_module)?;

        let Value::I32(result_pointer) = result_pointer_value[0] else {
            return Err(WingsError::from_invalid_module(
                "Describe function returned wrong type",
            ));
        };
        let result_offset = result_pointer as usize;
        let mut total_len = [0; size_of::<u32>()];
        memory
            .read(&mut ctx, result_offset, &mut total_len)
            .map_err(WingsError::from_invalid_module)?;
    ...
    fn generate_system_trait_map(
        &self,
        image: &WingsImage,
    ) -> FxHashMap<DisjointExportedType, ExportedType> {
        let mut result: FxHashMap<DisjointExportedType, ExportedType> =
            HashMap::with_capacity_and_hasher(image.systems.len(), FxBuildHasher::default());
        for system in image.systems.values() {
            let descriptor = &image.modules[system.module_id as usize]
                .0
                .metadata
                .system_descriptors[system.system_id as usize];
            for system_trait in &descriptor.traits {
                match result.entry(DisjointExportedType::from(&system_trait.ty)) {
                    hash_map::Entry::Occupied(mut v) => {
                        if v.get().version < descriptor.ty.version {
    ...
    fn sort_image_dependencies(
        &self,
        image: &WingsImage,
    ) -> Result<Vec<DisjointExportedType>, WingsError> {
        let trait_map = self.generate_system_trait_map(image);
        let mut sort = TopologicalSort::new();

        let mut to_process = image
            .top_level_systems
            .iter()
            .map(DisjointExportedType::from)
            .filter(|x| image.systems.contains_key(x))
            .collect::<Vec<_>>();
        let mut result =
            HashSet::with_capacity_and_hasher(image.systems.len(), FxBuildHasher::default());
        while let Some(next) = to_process.pop() {
            ...
            if result.insert(next.clone()) {
                let descriptor = &image.modules[system.module_id as usize]
                    ...
                    .metadata
                    .system_descriptors[system.system_id as usize];
                for dependency_ty in descriptor
                    .dependencies
                    .iter()
                    .map(DisjointExportedType::from)
    ...
    fn create_host_imports<C: AsContextMut<UserState = WingsHostInner<H>, Engine = H::Engine>>(
        mut ctx: C,
        index: usize,
    ) -> Imports {
        let mut ctx_handle =
            take(&mut ctx.as_context_mut().data_mut().ctx).expect("Failed to get context");
        let mut imports = H::create_imports(&mut ctx_handle, &mut ctx);
        ctx.as_context_mut().data_mut().ctx = Some(ctx_handle);

        imports.define(
            "env",
            "__wings_invoke_proxy_function",
            Extern::Func(Self::create_invoke_proxy_func(&mut ctx, index)),
        );
        imports.define(
            "env",
            "__wings_proxy_index",
            Extern::Func(Self::create_proxy_index_func(&mut ctx, index)),
        );
        imports.define(
            "env",
            "__wings_raise_event",
            Extern::Func(Self::create_raise_event_func(&mut ctx, index)),
        );
    ...
    fn create_invoke_proxy_func<
        C: AsContextMut<UserState = WingsHostInner<H>, Engine = H::Engine>,
    >(
        mut ctx: C,
        index: usize,
    ) -> Func {
        Func::new(
            &mut ctx,
            FuncType::new([ValueType::I32; 4], []),
            move |mut ctx, params, _| {
                let [Value::I32(id), Value::I32(func_index), Value::I32(pointer), Value::I32(size)] =
                    params
                else {
                    unreachable!()
                };
                ...
                match data.system_traits.get(*id as usize).copied() {
                    Some(SystemTraitHolder::Host { invoker }) => (data.invokers[invoker as usize]
                        .invoke)(
                        data.ctx.as_mut().expect("Failed to get context"),
                        *func_index as u32,
                        &mut buffer,
                    )?,
                    Some(SystemTraitHolder::Guest {
                        invoke,
                        system,
                        v_table,
                    }) => Self::invoke_guest_proxy(
                        &mut ctx,
                        *func_index as u32,
                        system,
                        invoke,
                ...
                let mut result = [Value::I32(0)];
                let alloc_marshal_buffer = ctx.data().instance_funcs[index]
                    .alloc_marshal_buffer
                    .clone();
                alloc_marshal_buffer.call(
                    &mut ctx,
    ...
    >(
        ...
        Func::new(
            &mut ctx,
            FuncType::new([ValueType::I32; 2], [ValueType::I32]),
            move |mut ctx, params, results| {
                let [Value::I32(pointer), Value::I32(size)] = params else {
                    unreachable!()
                };
                let len = *size as usize;
                with_marshal_slice(len, |buffer| {
                    ...
                    let ty =
                        DisjointExportedType::from(bincode::deserialize::<ExportedType>(buffer)?);
                    ...
                    results[0] = Value::I32(
                        *data
                            .system_trait_map
                            .get(&ty)
                            .ok_or_else(|| WingsError::from_trap("Invalid proxy type"))?
                            as i32,
                    );
    ...
    fn create_raise_event_func<C: AsContextMut<UserState = WingsHostInner<H>>>(
        mut ctx: C,
        index: usize,
    ) -> Func {
        Func::new(
            &mut ctx,
            FuncType::new([ValueType::I32; 2], []),
            move |mut ctx, params, _| {
                let [Value::I32(pointer), Value::I32(size)] = params else {
                    unreachable!()
                };
                ...
                with_marshal_slice(len, |buffer| {
                    ...
                    let ty = DisjointExportedType::from(bincode::deserialize::<ExportedType>(
                        section_reader.section()?,
                    )?);
                    let inner = ctx.data_mut();
                    if let Some(raiser) = inner.event_raisers.get(&ty) {
                        raiser(
                            inner.ctx.as_mut().expect("Failed to get context"),
                            section_reader.section()?,
                        )?;
                    } else {
                        inner
    ...
    fn get_instance_funcs<C: AsContextMut>(
        mut ctx: C,
        instance: &Instance,
    ) -> Result<InstanceFuncs, WingsError> {
        let alloc_marshal_buffer = Self::get_instance_func(
            &mut ctx,
            instance,
            "__wings_alloc_marshal_buffer",
        ...
        let invoke_func_1 = Self::get_instance_func(
            &mut ctx,
            instance,
            "__wings_invoke_func_1",
            FuncType::new([ValueType::I32; 2], [ValueType::I32]),
        )?;
        let invoke_func_2 = Self::get_instance_func(
            &mut ctx,
            instance,
            "__wings_invoke_func_2",
            FuncType::new([ValueType::I32; 3], [ValueType::I32]),
        )?;
        let invoke_proxy_func = Self::get_instance_func(
            &mut ctx,
            instance,
            "__wings_invoke_proxy_func",
            FuncType::new([ValueType::I32; 4], [ValueType::I32]),
        )?;
    ...
    fn get_instance_func<C: AsContextMut>(
        mut ctx: C,
        instance: &Instance,
        name: &str,
        ty: FuncType,
    ) -> Result<Func, WingsError> {
        let Some(Extern::Func(func)) = instance.get_export(&mut ctx, name) else {
            return Err(WingsError::from_invalid_module(format!(
                "Module missing intrinsic {name}"
            )));
        };
        ...
        if func.ty(&mut ctx) == ty {
            ...
            Err(WingsError::from_invalid_module(format!(
                "Module intrinsic {name} had invalid signature: {:?}",
                func.ty(&mut ctx)
            )))
        }
    }
    ...
    fn handle_error(&mut self, error: WingsError) {
        self.store
            ...
            .as_ref()
            .expect("Failed to get context")
            .raise_event(on::Error { error });
        self.reset_store();
    }

    /// Invokes a proxy function within a guest instance.
    fn invoke_guest_proxy(
        mut ctx: &mut StoreContextMut<WingsHostInner<H>, H::Engine>,
        func_index: u32,
        system: u32,
        invoke: GuestPointer,
        ...
        let Some(system) = ctx.data().systems.get(system as usize).copied() else {
            return Err(WingsError::from_trap("Invalid system ID"));
        };
        ...
        instance_funcs
            .alloc_marshal_buffer
            .call(&mut ctx, &[Value::I32(buffer.len() as i32)], &mut result)
            .map_err(WingsError::from_trap)?;
        let [Value::I32(pointer)] = result else {
            unreachable!()
        };
        memory
            .write(&mut ctx, pointer as usize, buffer)
            .map_err(WingsError::from_trap)?;
        ...
        instance_funcs
            ...
            .call(
                &mut ctx,
                &[
                    Value::I32(invoke.into()),
                    Value::I32(system.pointer.into()),
                    Value::I32(v_table.into()),
                    Value::I32(func_index as i32),
                ],
                &mut result,
            )
            .map_err(WingsError::from_trap)?;
        let [Value::I32(size)] = result else {
            unreachable!()
        };

        instance_funcs
            .alloc_marshal_buffer
            .call(&mut ctx, &[Value::I32(size)], &mut result)
            .map_err(WingsError::from_trap)?;
        let [Value::I32(pointer)] = result else {
            unreachable!()
        };
        ...
        memory
            .read(&mut ctx, pointer as usize, &mut buffer[..len])
            .map_err(WingsError::from_trap)?;
    ...
    fn dispatch_event<
        T: wings_marshal::exported_type::ExportType + Serialize + DeserializeOwned,
    >(
        &mut self,
        event: &T,
    ) {
        if let Err(error) = bincode::serialize(&event)
            .map_err(WingsError::Serialization)
            .and_then(|x| self.dispatch_raw_event(&ExportedType::from(T::TYPE).into(), &x))
        {
    ...
    fn dispatch_raw_event(
        &mut self,
        ty: &DisjointExportedType,
        buffer: &[u8],
    ) -> Result<(), WingsError> {
        if let Some(handler_list) = self
            .store
            .as_ref()
            .expect("Failed to get store")
            .data()
    ...
    fn lower_raw_event_buffer(
        &mut self,
        handler_list: &EventHandlerList,
        buffer: &[u8],
    ) -> Result<(), WingsError> {
        let mut store = self.store.as_mut().expect("Failed to get store");
        ...
        for instance in handler_list.instances.iter().copied() {
            ...
            let mut result = [Value::I32(0)];
            alloc_marshal_buffer
                .call(&mut store, &[Value::I32(buffer.len() as i32)], &mut result)
                .map_err(WingsError::from_trap)?;
            let [Value::I32(pointer)] = result else {
                unreachable!()
            };
            ...
            memory
                .write(&mut store, pointer as usize, buffer)
                .map_err(WingsError::from_trap)?;
        ...
        for handler in &handler_list.event_handlers {
            ...
            invoke_func
                .call(
                    &mut store,
                    &[
                        Value::I32(handler.invoke_func.into()),
                        Value::I32(system.pointer.into()),
                        Value::I32(handler.event_func.into()),
                    ],
                    &mut [Value::I32(0)],
                )
                .map_err(WingsError::from_trap)?;
    ...
    fn get_event_raisers() -> FxHashMap<DisjointExportedType, HostEventRaiserFunc<H>> {
        FxHashMap::from_iter(
            H::EVENTS
                .event_raisers
                .iter()
                .map(|x| (ExportedType::from(x.ty).into(), x.raise)),
        )
    }
...
impl<H: Host> GeeseSystem for WingsHost<H> {
    ...
    const EVENT_HANDLERS: geese::EventHandlers<Self> =
        H::EVENTS.event_handlers.with(Self::dispatch_guest_event);
    ...
    fn new(mut ctx: GeeseContextHandle<Self>) -> Self {
        ...
        let store = Some(wasm_runtime_layer::Store::new(
            &engine,
            Self::create_host_inner(ctx),
        ));

        Self { engine, id, store }
    }
}
...
pub trait Host: 'static + Sized {
    ...
    #[allow(unused)]
    fn create_imports(
        ctx: &mut GeeseContextHandle<WingsHost<Self>>,
...
type HostEventRaiserFunc<H> =
    fn(&GeeseContextHandle<WingsHost<H>>, &[u8]) -> Result<(), WingsError>;
...
type HostInvokerFunc<H> =
    fn(&mut GeeseContextHandle<WingsHost<H>>, u32, &mut Vec<u8>) -> Result<(), WingsError>;
...
mod private {
    ...
    pub trait HostEvent:
        wings_marshal::exported_type::ExportType + Serialize + DeserializeOwned + Send + Sync
    {
    }

    impl<
            T: wings_marshal::exported_type::ExportType + Serialize + DeserializeOwned + Send + Sync,
        > HostEvent for T
    {
    }
}
